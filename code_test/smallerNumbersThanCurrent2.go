...
*******************************************************************************/
func smallerNumbersThanCurrent2(anums []int, u32anumsSize int, aret []int) int {
	if u32anumsSize > 255 {
...
func main() {
	anums := []int{8, 1, 2, 2, 3}
	aret := make([]int, len(anums)) // 根据实际输入长度申请内存
	result := smallerNumbersThanCurrent2(anums, len(anums), aret)

	if result == 0 {
		fmt.Println(aret) // 输出: [4, 0, 1, 1, 3]
	} else {
		fmt.Println("执行失败")
	}
}
